...
protected:
	...
	size_t code = 404;
	std::map<std::string, std::string> header{ {CONTENT_TYPE, TEXT_PLAIN},{CONNECTION,KEEP_ALIVE} };
	std::string body{};
...
public:
	...
	void SetCode(size_t c) {
		...
		case 404:
			...
			break;
		case 206:
			this->desc = PARTIAL_CONTENT;
			break;
		case 416:
			this->desc = REQUESTED_RANGE_NOT_SATISFIABLE;
			break;
		default:
			this->desc = RESPONSE_DESC_OK;
		}
	...
	void SetFp(const std::string& path) {
		...
		this->header[ACCEPT_RANGES] = BYTES;
	}
